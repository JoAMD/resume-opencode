...
describe('runOpenCode session lifecycle (opt-in OPENCODE_KEEP_SESSION=false)', () => {
  ...
  it('deletes the session after a successful resume generation', async () => {
    const { generateResumeJSON } = await loadModule({ OPENCODE_KEEP_SESSION: 'false' });
  ...
  it('rotation: rotates the client after OPENCODE_CLIENT_ROTATE_AFTER requests and reuses session.delete', async () => {
    ...
    const { generateResumeJSON } = await loadModule({ OPENCODE_CLIENT_ROTATE_AFTER: '2', OPENCODE_KEEP_SESSION: 'false' });
...
describe('runOpenCode session lifecycle (default OPENCODE_KEEP_SESSION=true)', () => {
  let tmpLogDir: string;
  let savedEnv: Record<string, string | undefined>;

  beforeEach(() => {
    tmpLogDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ai-lifecycle-keep-'));
    savedEnv = {
      OPENCODE_MODEL: process.env.OPENCODE_MODEL,
      OPENCODE_MODEL_PROVIDER_ID: process.env.OPENCODE_MODEL_PROVIDER_ID,
      OPENCODE_MODEL_ID: process.env.OPENCODE_MODEL_ID,
  ...
  it('does not call session.delete after a successful resume generation', async () => {
  ...
  it('does not call session.delete when the prompt throws', async () => {
    const { generateResumeJSON } = await loadModule();
    mockClient.session.prompt.mockImplementationOnce(async (opts: any) => {
      sessionCalls.push({ method: 'prompt', sessionId: opts?.path?.id });
      throw new Error('simulated prompt failure');
  ...
  it('rotation: rotates the client after OPENCODE_CLIENT_ROTATE_AFTER requests without leaking session.delete', async () => {
    const sdkModule = await import('@opencode-ai/sdk');
